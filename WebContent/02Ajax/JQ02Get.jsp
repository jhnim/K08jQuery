    	...
    	jQuery에서 제공하는 Ajax관련 메소드
    		2. $.get()
    			...
    			형식] $.get(url, param, callback);
    				...
    				콜백메소드가 호출될때 전달되는 파라미터를 통해 성공 혹은 실패여부를 판단할 수 있다.
    			※ 서버로 요철시 파라미터가 없는 경우 생략가능함
    */
    $(function() {
		$('#btnXml').click(function(){
			$.get(
				...
				function(data){//콜백메소드
					...
					//alert(data);
					console.log(data);
					//콜백해준 내용을 대상으로 피싱(Parsing)을 진행한다.
					/*  
					...
					*/
					$(data).find("명함").each(function(){
						...
						/*  
						해당 영역의 내용을 empty()로 삭제한 후 append()로 내용을 추가한다.
						html()과 동일한 기능이 수행된다.
						*/
						$('#xmlDisplay').empty();
		...
		/*
		서버로 요청시 파라미터가 필요한 경우 JSON으로 만들어서 사용하게 된다.
